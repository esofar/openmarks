export default [
    {
        name: '用心精选',
        icon: 'heart',
        items: [
            {
                title: 'Google',
                desc: '全球最大的机器搜索引擎',
                logo: './logos/google.png',
                href: 'https://www.google.com',
                sort: 1000,
            },
            {
                title: 'Github',
                desc: '全球最大的开源代码托管平台',
                logo: './logos/github.png',
                href: 'https://github.com',
                sort: 1000,
            },
            {
                title: 'YouTube',
                desc: '全球最大的视频分享网站',
                logo: './logos/youtube.png',
                href: 'https://www.youtube.com',
                sort: 1000,
            },
            {
                title: 'Stack Overflow',
                desc: '程序设计领域的问答网站',
                logo: './logos/stackoverflow.png',
                href: 'https://stackoverflow.com',
                sort: 1000,
            },
            {
                title: 'Medium',
                desc: '基于主题的高质量阅读写作平台',
                logo: './logos/medium.png',
                href: 'https://medium.com',
                sort: 1000,
            },
        ]
    },
    {
        name: '在线学习',
        icon: 'read',
        items: [
            {
                title: 'Class Central',
                desc: '免费在线课程和慕课搜索引擎',
                logo: './logos/classcentral.png',
                href: 'https://www.classcentral.com',
                sort: 1000,
            },
            {
                title: 'Coursera',
                desc: '致力于普及全世界最好的教育，它与全球一流大学和机构合作提供在线课程',
                logo: './logos/coursera.png',
                href: 'https://www.coursera.org',
                sort: 1000,
            },
            {
                title: 'Edx',
                desc: '麻省理工和哈佛大学联手创建的大规模开放在线课堂平台，是唯一一个对公众开放学习资源的非营利性慕课平台',
                logo: './logos/edx.png',
                href: 'https://www.edx.org',
                sort: 1000,
            },
            {
                title: 'LeetCode',
                desc: '世界领先的在线编程学习平台',
                logo: './logos/leetcode.png',
                href: 'https://leetcode.com',
                sort: 1000,
            },
            {
                title: 'GeekTime',
                desc: '极客邦科技出品的一款 IT 内容知识服务 App，内容包含专栏订阅、极客新闻、热点专题、直播、视频和音频等多形式的知识内容',
                logo: './logos/geekbang.png',
                href: 'https://time.geekbang.org',
                sort: 1000,
            },
        ]
    },
    {
        name: '静态网站',
        icon: 'build',
        items: [
            {
                title: 'Hugo',
                desc: '使用 Go 语言实现的静态网站生成器，全球构建网站速度最快的框架。以惊人的速度和灵活性让网站构建变得更有趣。',
                logo: './logos/gohugo.png',
                href: 'https://gohugo.io',
                sort: 1000,
            },
            {
                title: 'Hexo',
                desc: '使用 Node.js 语言实现的快速、简洁且高效的博客框架。主题繁多又漂亮好看，但文章数量上去渲染速度远不及 Hugo。',
                logo: './logos/hexo.png',
                href: 'https://hexo.io',
                sort: 1000,
            },
            {
                title: 'Jekyll',
                desc: '使用 Ruby 语言实现的静态站点生成器，GitHub Pages 官方支持。',
                logo: './logos/jekyll.png',
                href: 'https://jekyllrb.com',
                sort: 1000,
            },
            {   
                title: 'Docsify',
                desc: '一个神奇的文档站点生成器，简单轻量、无需构建、支持多主题，非常适合快速搭建小型文档网站。',
                logo: './logos/docsify.png',
                href: 'https://docsify.js.org',
                sort: 1000,
            },
            {
                title: 'VuePress',
                desc: 'Vue 驱动的静态网站生成器。',
                logo: './logos/vuepress.png',
                href: 'https://vuepress.vuejs.org',
                sort: 1000,
            },
            {
                title: 'GitBook',
                desc: '一个现代化的文档平台，团队可以记录从产品到 API 和内部知识库的所有内容，开源项目和非营利团队可以申请免费使用。',
                logo: './logos/gitbook.png',
                href: 'https://www.gitbook.com',
                sort: 1000,
            },
            {
                title: 'Read the Docs',
                desc: '创建、托管和浏览文档，提供自动构建、版本控制、免费托管等服务，从而简化了软件文档。',
                logo: './logos/readthedocs.png',
                href: 'https://readthedocs.org',
                sort: 1000,
            },
            {
                title: 'Gridea',
                desc: '一个静态博客写作客户端，可以用它来记录你的生活、心情、知识、笔记、创意。',
                logo: './logos/gridea.png',
                href: 'https://gridea.dev',
                sort: 1000,
            },
        ]
    },
];